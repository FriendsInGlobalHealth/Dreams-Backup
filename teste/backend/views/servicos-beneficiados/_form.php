...
['class' => 'form-control',
 'onchange'=>'$.post("listas.dreams?id='.'"+$(this).val(), function(data) {
  ...
  echo Html::activeDropDownList($model, 'servico_id', ArrayHelper::map(ServicosDream::find()->where(['=','servico_id',$_REQUEST['ts']])->all(), 'id', 'name'),
['class' => 'form-control',
 'onchange'=>'$.post("listas.dreams?id='.'"+$(this).val(), function(data) {
    $("select#servicosbeneficiados-sub_servico_id").html(data);
 });',
...
<div class="col-lg-4">        <?= $form->field($model, 'sub_servico_id')->widget(Select2::classname(), [
    'data' => ArrayHelper::map(SubServicosDreams::find()->orderBy('name ASC')->where('status IS NOT NULL')->asArray()->all(), 'id', 'name')
]);
?>
</div>	
...
?>
 ...
 ?>
	...
	<?php $form->field($model, 'activista_id')->textInput() ?> </div> 
	 <div class="col-lg-12">
