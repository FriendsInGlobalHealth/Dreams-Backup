...
INNER JOIN sa_status_intervention ON sa_status_intervention.status_intervention_id = sa_action_plan1.status_intervention_id 

INNER JOIN sa_period_plan ON sa_period_plan.period_id = sa_action_plan1.action_plan_period 
...
foreach($result as $row)
{
	...
	$sub_array[] = $row['status_intervention_name'];	
	$sub_array[] = $row['action_plan_updaty'];
	$sub_array[] = $row['coment'];		
	
	$sub_array[] = $status;
	$sub_array[] = '<button type="button" name="update" id="'.$row["action_plan_id"].'" class="btn btn-warning btn-xs update">Actualizar</button>';
