    		...
    		<form method="post" id="cargo_form">
    			<div class="modal-content">
    				<div class="modal-header">
						...
						<h4 class="modal-title"><i class="fa fa-plus"></i> Add Cargo</h4>
    				</div>
    				<div class="modal-body">
    					<label>Enter Cargo Name</label>
						<input type="text" name="cargo_name" id="cargo_name" class="form-control" required />
    				</div>
...
$(document).ready(function(){
	...
	$(document).on('click', '.update', function(){
		...
		$.ajax({
			url:"cargo_action.php",
			method:"POST",
			data:{cargo_id:cargo_id, btn_action:btn_action},
			dataType:"json",
			success:function(data)
			{
	...
	$(document).on('click', '.delete', function(){
		...
		if(confirm("Are you sure you want to change status?"))
		{
			$.ajax({
				url:"cargo_action.php",
				method:"POST",
				data:{cargo_id:cargo_id, status:status, btn_action:btn_action},
				success:function(data)
				{
