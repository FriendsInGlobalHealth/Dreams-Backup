...
$(document).ready(function(){  
    ...
    function fetch_data()
    {
        $.ajax({
            ...
            success:function(data)
            {
                ...
                {
                    html += '<tr>';
                    html += '<td><input type="checkbox" id="'+data[count].id+'"data-data_avaliacao="'+data[count].data_avaliacao+'"data-nome_avaliacao="'+data[count].nome_avaliacao+'"data-instancia_id="'+data[count].instancia_id+'"data-tipo_avaliacao_id="'+data[count].tipo_avaliacao_id+'"data-estado="'+data[count].estado+'"data-data_fim_avaliacao="'+data[count].data_fim_avaliacao+'"class="check_box"/></td>';
    ...
    $(document).on('click', '.check_box', function(){
        ...
        if(this.checked)
        {
            html = '<td><input type="checkbox" id="'+$(this).attr('id')+'"data-data_avaliacao="'+$(this).data('data_avaliacao')+'"data-nome_avaliacao="'+$(this).data('nome_avaliacao')+'"data-instancia_id="'+$(this).data('instancia_id')+'"data-tipo_avaliacao_id="'+$(this).data('tipo_avaliacao_id')+'"data-estado="'+$(this).data('estado')+'"data-data_fim_avaliacao="'+$(this).data('data_fim_avaliacao')+'"class="check_box" checked /></td>';
            ...
            html += '<td><select name="estado[]" id="gender_'+$(this).attr('id')+'" class="form-control"><option value="1">SIM</option><option value="0">Nao</option></select></td>';			
            html += '<td><input type="date" name="data_fim_avaliacao[]" class="form-control" value="'+$(this).data("data_fim_avaliacao")+'" /><input type="hidden" name="hidden_id[]" value="'+$(this).attr('id')+'" /></td>';
        }
        else
        {
        ...
        $('#gender_'+$(this).attr('id')+'').val($(this).data('estado'));
    });
